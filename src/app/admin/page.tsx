...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  const confirmedCount = inquiries.filter((inquiry) => ["confirmed", "paid"].includes(inquiry.status)).length;
  const paidCount = inquiries.filter((inquiry) => inquiry.paymentStatus === "success" || inquiry.status === "paid").length;
  const estimatedRevenue = inquiries.reduce(
    (sum, inquiry) => sum + estimateInquiryRevenue(inquiry.destination, inquiry.travelers),
    0,
  );
    ...
    <main className="min-h-screen bg-[#f7f4ee] text-[#18211f]">
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-5 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="flex items-center gap-4">
            <Image
              ...
              width={58}
              height={58}
              className="h-14 w-14 rounded-[8px] bg-white object-contain p-1 shadow-sm ring-1 ring-[#e1d8c8]"
            />
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#276457]">Sakura operations</p>
              <h1 className="mt-1 text-2xl font-semibold sm:text-3xl">Захиалга, төлбөр, хэрэглэгчийн самбар</h1>
            </div>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link href="/" className="inline-flex h-11 items-center justify-center rounded-[8px] bg-[#276457] px-5 font-semibold text-white">
              Вэбсайт
            </Link>
            <form action={logoutAdmin}>
              <button className="inline-flex h-11 items-center justify-center rounded-[8px] border border-[#d8cebd] bg-white px-5 font-semibold text-[#34443e]">
                Гарах
              </button>
            </form>
          </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 py-6 sm:grid-cols-2 sm:px-5 lg:grid-cols-4 lg:px-8">
        {stats.map(({ icon: Icon, label, value, hint }) => (
          <div key={label} className="group relative overflow-hidden rounded-[8px] bg-white p-5 shadow-sm ring-1 ring-[#e1d8c8] transition hover:-translate-y-1 hover:shadow-xl hover:shadow-[#7b481c]/10">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="grid h-11 w-11 place-items-center rounded-full bg-[#eef8f3] text-[#276457]">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-6 sm:px-5 lg:px-8">
        <div className="grid gap-4 rounded-[8px] border border-[#e1d8c8] bg-white p-4 shadow-sm lg:grid-cols-[1fr_auto] lg:items-center">
          <form className="grid gap-3 sm:grid-cols-[1fr_180px_auto]">
            <label className="relative">
              ...
              <input
                ...
                placeholder="Нэр, утас, имэйл, код, аяллаар хайх"
                className="h-11 w-full rounded-[8px] border border-[#d8cebd] bg-[#fffaf0] pl-10 pr-4 outline-none focus:border-[#276457] focus:ring-2 focus:ring-[#276457]/15"
              />
            </label>
            <select
              name="status"
              defaultValue={activeStatus}
              className="h-11 rounded-[8px] border border-[#d8cebd] bg-[#fffaf0] px-3 outline-none focus:border-[#276457] focus:ring-2 focus:ring-[#276457]/15"
            >
          ...
          <button className="inline-flex h-11 items-center justify-center gap-2 rounded-[8px] border border-[#d8cebd] px-4 font-semibold text-[#34443e]">
                ...
                return (
                  <details key={inquiry.id} className="group">
                    <summary className="grid cursor-pointer list-none gap-4 p-5 transition hover:bg-[#fffaf0] lg:grid-cols-[0.9fr_1fr_1.15fr_0.72fr_0.95fr]">
                      <div>
                        <p className="font-mono text-sm font-semibold text-[#276457]">{inquiry.bookingCode ?? `#${inquiry.id}`}</p>
                        <p className="mt-1 text-sm text-[#6b716b]">{formatDate(inquiry.createdAt)}</p>
                      </div>
                      <div>
                        <p className="font-semibold">{inquiry.name}</p>
                        <p className="mt-1 text-sm text-[#6b716b]">{inquiry.phone}</p>
                      </div>
                      <div>
                        <p className="font-semibold">{inquiry.destination}</p>
                        <p className="mt-1 text-sm text-[#6b716b]">Явах: {formatDate(inquiry.preferredDate)}</p>
                      </div>
                      <div>
                        <p className="text-sm">{inquiry.travelers} хүн</p>
                        <p className="mt-1 text-sm text-[#6b716b]">Том {inquiry.adults}, хүүхэд {inquiry.children}</p>
                      </div>
                      <div className="flex flex-wrap items-start gap-2 lg:justify-end">
                        <span className="rounded-full bg-[#eef8f3] px-3 py-1 text-xs font-semibold text-[#276457]">
                          {statusLabels[inquiry.status] ?? inquiry.status}
                        </span>
                        <span className="rounded-full bg-[#ffe5f1] px-3 py-1 text-xs font-semibold text-[#b0184c]">
                          {paymentLabels[inquiry.paymentStatus] ?? inquiry.paymentStatus}
                        </span>
                      </div>
                    </summary>

                    <div className="grid gap-5 border-t border-[#ead9c4] bg-[#fffaf0] p-5 lg:grid-cols-[1fr_340px]">
                      <div className="grid gap-4 md:grid-cols-2">
                        <InfoCard title="Холбоо барих">
                          <a href={`tel:${inquiry.phone}`} className="mt-3 flex items-center gap-2 text-sm font-semibold text-[#276457]">
                            <Phone className="h-4 w-4" />
                            {inquiry.phone}
                          </a>
                          {inquiry.email ? (
                            <a href={`mailto:${inquiry.email}`} className="mt-2 flex items-center gap-2 text-sm font-semibold text-[#276457]">
                              <Mail className="h-4 w-4" />
                              {inquiry.email}
                            </a>
                          ) : null}
                        </InfoCard>
                        <InfoCard title="Төлбөр ба үнэ">
                          <p className="mt-3 text-sm text-[#5d655f]">Арга: {readable(inquiry.paymentMethod, paymentMethods)}</p>
                          <p className="mt-2 text-sm text-[#5d655f]">Төсөв: {inquiry.budget || "бичээгүй"}</p>
                          <p className="mt-2 text-xl font-semibold text-[#17211d]">{formatMoney(revenue)}</p>
                        </InfoCard>
                        <InfoCard title="Аяллын мэдээлэл">
                          <p className="mt-3 flex items-center gap-2 text-sm text-[#5d655f]">
                            <CalendarDays className="h-4 w-4 text-[#276457]" />
                            {formatDate(inquiry.preferredDate)}
                          </p>
                          <p className="mt-2 text-sm text-[#5d655f]">Виз: {inquiry.visaStatus || "асуугаагүй"}</p>
                          <p className="mt-2 text-sm text-[#5d655f]">Паспорт: {inquiry.passportStatus || "асуугаагүй"}</p>
                        </InfoCard>
                        <InfoCard title="Хэрэглэгчийн хүсэлт">
                          <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#5d655f]">
                            {inquiry.message || "Нэмэлт хүсэлт үлдээгээгүй."}
                          </p>
                        </InfoCard>
                      </div>
                      ...
                      <form action={updateInquiryStatus} className="rounded-[8px] border border-[#e1d8c8] bg-white p-4 shadow-sm">
                        ...
                        <label className="mt-4 block text-sm font-semibold text-[#34443e]">
                          Захиалгын төлөв
                          <select name="status" defaultValue={inquiry.status} className="mt-2 h-11 w-full rounded-[8px] border border-[#d8cebd] bg-white px-3">
                        ...
                        <label className="mt-4 block text-sm font-semibold text-[#34443e]">
                          Төлбөрийн төлөв
                          <select name="paymentStatus" defaultValue={inquiry.paymentStatus} className="mt-2 h-11 w-full rounded-[8px] border border-[#d8cebd] bg-white px-3">
                        ...
                        <button className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-[8px] bg-[#276457] font-semibold text-white">
...
function InfoCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-[8px] border border-[#e1d8c8] bg-white p-4 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#b0184c]">{title}</p>
      {children}
    </div>
  );
}
