...
import { ArrowLeft, CalendarDays, Clock, MapPin, Plane, TicketCheck } from "lucide-react";
...
function timeRemaining(date: Date | null) {
  if (!date) {
    return "Аяллын өдөр баталгаажаагүй байна";
  }

  const diff = date.getTime() - Date.now();

  if (diff <= -24 * 60 * 60 * 1000) {
    return "Аяллын хугацаа өнгөрсөн байна";
  }

  if (diff <= 0) {
    return "Аялал өнөөдөр эхэлнэ";
  }
  ...
  if (days === 0) {
    return `Аялал эхлэхэд ${hours} цаг үлдлээ`;
  }
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 flex flex-col justify-between gap-5 border-b border-white/12 pb-8 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-lg leading-8 text-white/70">
                Захиалгын төлөв, аяллын өдөр, үлдсэн хугацаагаа нэг дор хянаарай.
              </p>
            </div>
            <Link
              href="/booking"
              className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-[#e8b95e] px-6 font-semibold text-[#17211d] shadow-xl shadow-[#e8b95e]/20"
            >
          ...
          {bookings.length === 0 ? (
            <div className="mt-10 rounded-[8px] border border-white/12 bg-white/[0.06] p-8 shadow-2xl shadow-black/20 backdrop-blur-xl">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#e8b95e] text-[#17211d]">
              ...
              <p className="mt-3 max-w-xl leading-7 text-white/68">
                Tokyo-Fuji аяллын багцаа сонгоод захиалга үүсгэвэл энэ хэсэгт таны аяллын мэдээлэл харагдана.
              </p>
            </div>
          ) : (
            <div className="mt-10 grid gap-5">
              {bookings.map((booking) => (
                <article key={booking.id} className="overflow-hidden rounded-[8px] border border-white/12 bg-[#fff8e7] text-[#17211d] shadow-2xl shadow-black/20">
                  <div className="grid gap-0 lg:grid-cols-[1fr_0.72fr]">
                    <div className="p-6 sm:p-8">
                      <div className="flex flex-wrap items-center gap-3">
                        <span className="rounded-full bg-[#10261f] px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-[#e8b95e]">
                          {booking.bookingCode ?? "Pending"}
                        </span>
                        <span className="rounded-full bg-[#f4b7c9]/22 px-4 py-2 text-sm font-semibold text-[#9a2049]">
                          {booking.status === "new" ? "Менежер баталгаажуулна" : booking.status}
                        </span>
                      </div>
                      ...
                      <div className="mt-5 grid gap-3 text-sm font-semibold text-[#405148] sm:grid-cols-2">
                        <span className="inline-flex items-center gap-2">
                          <CalendarDays className="h-4 w-4 text-[#b9872f]" />
                          {formatDate(booking.preferredDate)}
                        </span>
                        <span className="inline-flex items-center gap-2">
                          <Clock className="h-4 w-4 text-[#b9872f]" />
                          {timeRemaining(booking.preferredDate)}
                        </span>
                        <span className="inline-flex items-center gap-2">
                          <MapPin className="h-4 w-4 text-[#b9872f]" />
                          {booking.travelers} хүн · {booking.paymentStatus === "pending" ? "Төлбөр хүлээгдэж байна" : booking.paymentStatus}
                        </span>
                      </div>
                    </div>
                    <div className="bg-[#10261f] p-6 text-[#fff8e7] sm:p-8">
                      <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#e8b95e]">Contact</p>
                      ...
                      <Link
                        href="/booking"
                        className="mt-7 inline-flex h-11 items-center justify-center rounded-full border border-[#e8b95e]/60 px-5 text-sm font-semibold text-[#e8b95e] transition hover:bg-[#e8b95e] hover:text-[#17211d]"
                      >
                        Захиалга шинэчлэх
                      </Link>
                    </div>
