...
import { parseBookingPrefill } from "@/lib/travel-logic";
...
export function BookingExperience({ initialTourSlug, initialEmail = "", initialPhone = "", prefillText = "" }: BookingExperienceProps) {
  const prefill = parseBookingPrefill(prefillText);
  const initialIndex = Math.max(0, destinations.findIndex((item) => item.slug === (prefill.tour || initialTourSlug)));
  ...
  return (
    <main className="relative isolate min-h-screen overflow-hidden bg-[#07120f] text-[#fff8e7]">
      <div key={`page-bg-${selectedTour.slug}`} className="booking-bg-reveal absolute inset-0 -z-30">
        <Image src={selectedTour.image ?? japanImages.fujiSakura} alt={selectedTour.title} fill sizes="100vw" className="object-cover object-center opacity-62" priority />
      </div>
      ...
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-5 sm:px-5 lg:px-8">
        <Link href="/" className="inline-flex h-11 w-fit items-center gap-2 rounded-full border border-white/14 bg-white/8 px-4 text-sm font-semibold text-white/86 shadow-sm backdrop-blur transition hover:bg-white/14">
        ...
        <div className="grid flex-1 gap-5 py-5 lg:grid-cols-[0.92fr_1.08fr] lg:items-stretch">
          <section className="relative min-h-[62vh] overflow-hidden rounded-[8px] border border-[#e8c77a]/18 bg-[#10201d]/72 text-white shadow-2xl shadow-black/35 backdrop-blur-xl lg:min-h-full">
            <div key={`panel-bg-${selectedTour.slug}`} className="booking-image-reveal absolute inset-0">
              <Image src={selectedTour.image ?? japanImages.fujiSakura} alt={selectedTour.title} fill sizes="(min-width: 1024px) 44vw, 100vw" className="object-cover object-center" />
            </div>
            ...
            <div className="relative z-10 flex min-h-[62vh] flex-col justify-between p-5 sm:p-8 lg:min-h-full">
              ...
              <div className="max-w-2xl">
                <p className="booking-kicker text-sm font-semibold uppercase tracking-[0.24em] text-[#f4b7c9]">{selectedTour.badge} route</p>
                <h1 key={`title-${selectedTour.slug}`} className="booking-title-reveal mt-4 max-w-xl text-5xl font-semibold leading-[0.9] text-[#fff8e7] sm:text-7xl">{selectedTour.shortTitle}</h1>
                <p className="mt-5 max-w-xl text-lg leading-8 text-white/78">{selectedTour.description}</p>
                <div className="mt-5 flex flex-wrap items-center gap-3 text-sm font-semibold text-white/78">
                  <span className="inline-flex items-center gap-2"><MapPin className="h-4 w-4 text-[#e8c77a]" />{selectedTour.route}</span>
                  <span className="text-[#e8c77a]">·</span>
                  <span>{selectedTour.duration}</span>
                  <span className="text-[#e8c77a]">·</span>
                  <span>{selectedTour.priceFrom}</span>
                </div>
                <button type="button" onClick={() => selectBySlug(selectedTour.slug)} className="magnetic-cta mt-7 inline-flex h-12 items-center justify-center gap-2 rounded-full bg-[#e8b95e] px-6 font-semibold text-[#17211d] shadow-2xl shadow-[#e8b95e]/25 transition hover:bg-[#f6cf7a]">
              ...
              <div className="mt-8">
                <div className="grid gap-3 sm:grid-cols-3">
                  {highlights.map(({ icon: Icon, label }) => (
                    <div key={label} className="booking-micro-card flex items-center gap-3 rounded-[8px] border border-white/10 bg-white/8 p-3 backdrop-blur">
                ...
                <div className="mt-5 flex items-end gap-3">
                  <button type="button" onClick={goPrev} className="hidden h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/16 bg-white/10 text-white backdrop-blur transition hover:bg-white/18 sm:inline-flex" aria-label="Өмнөх маршрут">
                  ...
                  <div className="booking-thumb-row flex min-w-0 flex-1 gap-3 overflow-x-auto pb-1">
                    {destinations.map((item, index) => {
                      ...
                      return (
                        <button key={item.slug} type="button" onClick={() => setActiveIndex(index)} className={`group relative h-32 min-w-[168px] overflow-hidden rounded-[8px] border text-left shadow-xl shadow-black/20 transition duration-300 sm:h-36 sm:min-w-[190px] ${active ? "border-[#e8b95e] ring-2 ring-[#e8b95e]/40" : "border-white/16 hover:border-white/42"}`}>
                  ...
                  <button type="button" onClick={goNext} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/16 bg-white/10 text-white backdrop-blur transition hover:bg-white/18" aria-label="Дараагийн маршрут">
          ...
          <BookingForm initialTourSlug={selectedTour.slug} selectedTourSlug={selectedTour.slug} onTourSelect={selectBySlug} initialEmail={initialEmail} initialPhone={initialPhone} prefillText={prefillText} />
        </div>
      </div>
    </main>
  );
